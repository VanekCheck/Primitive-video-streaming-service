  ...
  subscribe(streamingService: StreamingService): Subscription {
    ...
    if (
      this.subscriptions.some(
        (currentSubs) => currentSubs.streamingService === streamingService
      )
    ) {
      throw new Error(
        'The user is already subscribed to this streaming service'
      )
    }
...
class Subscription {
  ...
  private getRecommendation(filteredMostViewedShows: Show[]): Show {
    // sort before get a random one ??????
    filteredMostViewedShows.sort((a, b) => b.getDuration() - a.getDuration())

    // get random
    return filteredMostViewedShows[
      Math.floor(Math.random() * filteredMostViewedShows.length)
    ]
  }
}
...
class StreamingService {
  ...
  constructor(public name: string, public shows: Array<Show>) {
  ...
  public getMostViewedShowsOfYear(year: number): Show[] {
    ...
    // filter shows by year
    const showsOfTheCurrentYear = this.shows.filter(
      (currentShow) => currentShow.releaseDate.getFullYear() === year
    )
  ...
  public getMostViewedShowsOfGenre(genre: Genres): Show[] {
    // filter shows by year
    const showsOfTheCurrentGenre = this.shows.filter(
      (currentShow) => currentShow.genre === genre
    )
  ...
  private getMostViewed(filteredShows: Show[]): Show[] {
    if (filteredShows.length === 0) {
      throw new Error('There is no shows by current filter')
    }
    // create array [[showName, views ]] to sort by views
    const showsWithViews: Array<[Show, number]> = filteredShows.map((show) => [
      show,
...
class Movie extends Show {
  public constructor(name, genre, releaseDate) {
